...
可以对**任意数据类型**的数据使用 MIN 和 MAX 函数。因为字符型日期型也可以进行比较
...
#例题：查询每个部门的名字，location_id，员工数量和平均工资，并按平均工资降序
# 注意点1：所有部门，所以是外连接
# 注意点2：select中除组函数以外的字段，group by中必须存在才行
# 注意点3：count（*）展示是每一组中有多少条数据，在查询员工数量的时候，需要指定字段，因为是所有部门，存在有的部门没有员工，但是有会有id为null的记录
...
**过滤分组：HAVING子句**，在数据分组之后对分组之后的数据进行过滤
...
```sql
# 查询各个部门中的最高工资超过10000的部门
# 错误的,过滤条件中如果有聚合函数，用having替换where
SELECT   department_id, MAX(salary)
FROM     employees
WHERE   MAX(salary)>10000
GROUP BY department_id; 
# 正确的 ：用having，并且放在group by后面
SELECT   department_id, MAX(salary)
FROM     employees
GROUP BY department_id
...
```sql
FROM -> WHERE -> GROUP BY -> HAVING -> SELECT 的字段 -> DISTINCT（行去重） -> ORDER BY -> LIMIT

# 所以在select中起的别名，在order by中可以使用，在where中不可以使用，因为mysql用的是临时表，可以在group by中和having中可以使用select中创建的字段别名，但是不建议使用

# 执行完group by语句之后才会进行聚合函数的计算，所以在where中使用聚合函数会报错，因为还没有计算结果
```
...
# 所以说，筛选条件越靠前执行效率越高，也就是把不含聚合函数的过滤条件放在where中，含聚合函数的过滤条件放在having中
```

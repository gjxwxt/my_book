...
5、需要设置新的结束标记，用于创建标志以结束整个存储过程

```mysql
# 因为函数中间的语句以;隔开，但并不代表语句的结束，此时可以将结束符变更，最后函数结束后改回来
DELIMITER 新的结束标记
例如 delimiter % 
    end	% #代表着该存储过程的结束
    delimiter ; #最后要把结束标识符改回来
```
...
```mysql
DELIMITER //
#out 变量名 变量类型，以创建时的类型为主，此时的ms是局部变量
...
RETURNS INT
	...
	NOT DETERMINISTIC # not deterministic不是确定性的,代表输入同样的值输出不一定
	READS SQL DATA # reads sql data 只读sql
	SQL SECURITY DEFINER #sql security defined 只有创建者才能调用该函数
	COMMENT '查询部门平均工资'
BEGIN

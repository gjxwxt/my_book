...
```mysql
#方式1：首先分组算出每个部门的平均工资，然后作为单行查询算出最低的平均工资，最后在作为单行查询作为条件与部门的平均工资进行比较
SELECT department_id
FROM employees
GROUP BY department_id
HAVING AVG(salary) = (
...
```mysql
#方式2：将所有部门的平均工资作为多行查询进行返回，然后通过多行比较操作符进行比较，最低就是<= all
SELECT department_id
FROM employees
GROUP BY department_id
...
```sql
# 将各个部门的部门id和部门平均工资作为一张表，连接条件就是部门id然后进行筛选
...
在select中除了group by和limit中都可以写子查询

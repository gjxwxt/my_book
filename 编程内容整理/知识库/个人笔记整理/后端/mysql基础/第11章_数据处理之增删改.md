...
- 通过查询将查询到的数据插入的时候类型要一致，不一致最起码要保证向下兼容才行，比如要新表的name是varchar(15)，select的表中的字段varchar(20)，此时就要注意了，因为有可能选择插入的值中有可能会有不满足要求的值，导致插入失败
...
- 可以一次更新**多条**数据，where筛选几条改几条

- 如果需要回滚数据，需要保证在DML前，进行设置：**SET AUTOCOMMIT = FALSE;**因为dml操作默认执行一步commit一次

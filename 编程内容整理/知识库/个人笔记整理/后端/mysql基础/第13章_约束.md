...
（2）建表后，本列必须符合这个条件才能设置上约束，不符合约束添加不上
...
- 唯一性约束允许列值为空。也就是可以传null值，而且可以多次传null值，null与null不等
...
CREATE TABLE t_course(
	cid INT UNIQUE, #约束名默认为cid，也就是列名
...
CREATE TABLE USER(
 ...
 NAME VARCHAR(25),
 PASSWORD VARCHAR(16),
 -- 使用表级约束语法 给约束起名：uk_表名_列名，可以整体简写成unique(name,password)
...
create table 表名称(
    ...
    字段名  数据类型,
    #符合约束名是字段列表中第一个列的名
    unique key(字段列表) #字段列表中写的是多个字段名，多个字段名用逗号分隔，表示那么是复合唯一，即多个字段的组合是唯一的，也就是只要有一列不一致就可以
);
```
...
create table student_course(
    ...
    cid int,
    score int,
    unique key(sid,cid)  #复合唯一，保证一个学生一个课程
);
```
...
create table 表名称(
    ...
    字段名  数据类型,  
    字段名  数据类型,
    [constraint 约束名] primary key(字段名) #表级模式，起的约束名不起作用，为primary
);
```
...
create table employee(
	eid int auto_increment,#必须是主键或者唯一键列
    ename varchar(20)
);
...
create table employee(
	eid int primary key,
    ename varchar(20) unique key auto_increment #必须是整形
);
...
```mysql
create table 表名称(
	字段名  数据类型  primary key auto_increment,#设置
...
```mysql
# net stop mysql57 停止mysql57这个服务
...
MySQL 8.0将**自增主键的计数器持久化到`重做日志`中**。每次计数器发生改变，都会将其写入重做日志中。如果数据库重启，InnoDB会根据重做日志中的信息来初始化计数器的内存值。
...
create table emp(#从表
    ...
    deptid int,				#员工所在的部门
    foreign key (deptid) references dept(did)   #在从表中指定外键约束，引入了dept表中的did列，建立外键约束
);
...
insert into emp values(2,'李四',1005);#添加从表记录失败，因为主表中没有这条记录
...
```mysql
update emp set deptid = 1002 where eid = 1;#修改从表失败，因为主表没有这个值可以被引用

update dept set did = 1002 where did = 1001;#修改主表失败，因为1001被引用了，改之前需要先将从表中的关于1001的引用给删除才能再删除主表中的相关记录

update dept set did = 1002 where did = 1003;#修改主表成功  因为部门表的1003部门没有被emp表引用，所以可以修改
```

```mysql
delete from dept where did=1001; #删除主表失败，因为部门表did的1001字段已经被emp引用了，所以部门表的1001字段对应的记录就不能被删除
```
...
* `Cascade方式`：在父表上update/delete记录时，同步update/delete掉子表的匹配记录 ，例如将主表中的dept_id从10变为20，那么从表中关联dept_id=10的数据被删掉
* `Set null方式`：在父表上update/delete记录时，将子表上匹配记录的列设为null，但是要注意子表的外键列不能为not null  ，例如之前的删除改成将从表中引用的该列变为null
* `No action方式`：如果子表中有匹配的记录，则不允许对父表对应候选键进行update/delete操作 。默认方式，即严格模式
...
SHOW INDEX FROM 表名称; #查看某个表的索引名，删除其中的key_name
